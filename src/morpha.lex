...
<scan>"+"/_       { return(common_noun_stem()); }
<scan>({G}|"+")+/_ { common_noun_stem(); printf("+"); return(1);} /* may not work with morphg */
...
char up8(char c)
{ if (('a' <= c && c <= 'z') || ('\xE0' <= c && c <= '\xFE' && c != '\xF7')) {return c-('a'-'A');}
  else {return c;}
}
...
{ char w[64];
  ...
  if (f == NULL) { 
	  fprintf(stderr, "File with consonant doubling verb stems not found (\"%s\").\n", fn);
  }
  else
  { while (1)
